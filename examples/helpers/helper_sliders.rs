...
pub const DISABLED_COLOR: Color = Color::srgb(0.61, 0.07, 0.16);
...
pub fn spawn_vertical_slider_ui(label: &str, init_scaled: f32, min: f32, max: f32, commands: &mut ChildSpawnerCommands, assets: &AssetServer) {
    assert!(min <= max);
...
fn update_value_labels(
    mut sliders: Query<(&SliderValue, &ValueTextId, &SliderExtents, &mut SliderScaledValue), Changed<SliderValue>>,
